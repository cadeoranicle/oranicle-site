(function () {
    ...
    function renderLHSBlock() {
        return `
        <div class="sku401-lhs-block">
            <div class="sku401-title">Canonical Positioning</div>
            <div class="sku401-subtitle">
                Review the hospital position across the canonical dimensions.
            </div>
            <div class="sku401-viewer-host" id="sku401ViewerHost"></div>
        </div>
    `;
    }
    ...
    function renderSummaryBlock(summary = {}) {
        return `
            <div class="sku401-card sku401-summary-card" style="margin-top:12px; padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:10px;">
                <div class="sku401-title">Canonical Summary</div>

                <div class="sku401-meta-grid" style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px;">
                    <div class="sku401-meta-row">
    ...
    function renderAxisCards(axes = []) {
        return axes.map(axis => `
            <div class="sku401-card sku401-axis-card" style="margin-top:12px; padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:10px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    ...
                    <div class="sku401-metric-pill" style="font-size:11px; padding:4px 8px; border-radius:999px; border:1px solid rgba(255,255,255,0.12);">
                        ${buildStrengthLabel(axis.percentile)}
                    </div>
                </div>

                <div class="sku401-meta-grid" style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                    <div class="sku401-meta-row">
    ...
    function renderNarrativeBlock(summary = {}) {
        return `
            <div class="sku401-card sku401-narrative-card" style="margin-top:12px; padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:10px;">
                <div class="sku401-title">Structural Narrative</div>
    ...
    function renderViewer(data = {}) {
        console.log("sku401 renderViewer CALLED", data);
        ...
        if (!targetEl) {
            console.warn("SKU4.01 missing targetEl");
            return;
        }

        targetEl.innerHTML = `
        <div class="sku401-shell">
            <div class="sku401-left">
                ${renderLHSBlock()}
            </div>
        </div>
    `;

        const viewerHost = targetEl.querySelector("#sku401ViewerHost");
        const traces = buildViewerTraces(payloadData, shouldShow);

        console.log("sku401 payloadData", payloadData);
        console.log("about to render SKU4.01 viewer traces", traces);
        ...
        if (viewerHost && window.Plotly) {
            ...
            console.warn("sku401: viewerHost or Plotly missing");
        }
    }
    ...
    function renderNarrative(data = {}) {
        ...
        if (!targetEl) {
            console.warn("SKU4.01 missing narrative targetEl");
            return;
        }

        targetEl.innerHTML = `
        <div class="sku401-panel4-wrap">
    ...
    function renderNotes(data = {}) {
        ...
        if (!targetEl) {
            console.warn("SKU4.01 missing notes targetEl");
            return;
        }
