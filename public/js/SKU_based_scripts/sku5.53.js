console.log("[SKU5.53.js] file loaded");

(function initSKU553Renderer(global) {
    "use strict";

    function render({ targetEl, payload, contract }) {
        console.log("[SKU5.53] render()", {
        ...
        const peerRows = Array.isArray(comparablePayload.peer_points)
            ...
            : [];
        const hospitalPoint = comparablePayload.hospital_point || null;
        ...
        if (hospitalPoint) {
            traces.push({
                ...
                hovertemplate:
                    ...
                    "NPI: " + String(hospitalPoint.npi || "") + "<br>" +
                    "X: %{x:.2f}<br>" +
                    "Y: %{y:.2f}<br>" +
                    "Z: %{z:.2f}<extra></extra>",
                showlegend: false,
    ...
    // RHS renderer SKU5.53 start //
    function renderRHSBlock({ targetEl, payload, contract }) {
        if (!targetEl) return;

        const comparablePayload = payload || {};
        const peerRows = Array.isArray(comparablePayload.peer_table_rows)
            ? comparablePayload.peer_table_rows
            : [];
        const percentiles = Array.isArray(comparablePayload.peer_cohort_percentiles)
            ? comparablePayload.peer_cohort_percentiles
            : [];

        let html = `
        <div class="sku553-rhs-section">
            <div class="sku553-section-header">
                <div class="sku553-section-title">Top Comparable Providers</div>
                <div class="sku553-section-subtitle">
                    Nearest comparable providers in canonical space
                </div>
            </div>
    `;
        ...
        if (peerRows.length) {
            html += `
            <div class="sku553-peer-table-wrap">
                <table class="sku553-peer-table">
                    <thead>
                        <tr>
                            <th>Rank</th>
                            <th>Comparable Provider</th>
                            <th>NPI</th>
                            <th>Similarity</th>
                            <th>Revenue</th>
                            <th>Intensity</th>
        ...
        `;
            ...
            html += `<div class="sku5-empty-state">No comparable providers available.</div>`;
        }

        html += renderPeerCohortPercentiles(percentiles);
        html += `</div>`;

        targetEl.innerHTML = html;
    }
    // RHS renderer SKU5.53 end //
